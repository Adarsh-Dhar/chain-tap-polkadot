...
const DISCOUNT_LABEL = "Auto 10% Off"

// CORS headers for checkout UI extension
const corsHeaders = {
...
export async function POST(req: Request) {
  try {
    ...
    console.log("🎫 Discount API called:", { shop, body })
    ...
    if (!sanitizeShop(cleanShop)) {
      console.error("❌ Invalid shop domain format:", cleanShop)
      return Response.json(
    ...
    console.log("🔍 Getting access token for shop:", cleanShop)
    const accessToken = await getAccessToken(cleanShop)
    if (!accessToken) {
      console.error("❌ No access token found for shop:", cleanShop)
      // Build authentication URL
      const authUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'https://91ed22d78410.ngrok-free.app'}/api/auth?shop=${encodeURIComponent(cleanShop)}`
      return Response.json(
        { 
          error: "No valid access token found. Please authenticate first.",
          details: "The app needs to be installed and authenticated in the Shopify store.",
          authUrl: authUrl,
          message: `Please install and authenticate the app by visiting: ${authUrl}`
        },
    ...
    const discountCode = `AUTO10-${Date.now()}-${Math.random().toString(36).substring(2, 8).toUpperCase()}`

    // Convert discount percentage to percentage value (e.g., 0.225 -> 22.5)
    const percentageValue = DISCOUNT_PERCENTAGE * 100
    ...
    const priceRuleData = {
      price_rule: {
        title: `${DISCOUNT_LABEL} ${percentageValue.toFixed(0)}%`,
    ...
    if (!priceRuleResponse.ok) {
      ...
      return Response.json(
        ...
        { status: priceRuleResponse.status, headers: corsHeaders }
      )
    }
    ...
    if (!discountCodeResponse.ok) {
      ...
      return Response.json(
        ...
        { status: discountCodeResponse.status, headers: corsHeaders }
      )
    }
    ...
    return Response.json({
      ...
      priceRuleId,
      percentageValue,
      message: `Automatic ${percentageValue.toFixed(0)}% discount is ready to use`,
    }, { headers: corsHeaders })
  } catch (error) {
